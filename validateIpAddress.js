...
*IPv4 addresses are canonically represented in dot-decimal notation, which consists of four decimal numbers, each ranging from
...
*separated by colons (":"). For example, the address 2001:0db8:85a3:0000:0000:8a2e:0370:7334 is a valid one. 
...
var validIPAddress = function(IP) {
    let ipV4RegX = /^(?:(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\.){3}(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])$/;
        if(ipV4RegX.test(IP)) return "IPv4";
   let ipV6RegX =/^(?:(?:[a-fA-F1-9][a-fA-F0-9]?[a-fA-F0-9]?[a-fA-F0-9]?|0)\:)(?:(?:[a-fA-F0-9][a-fA-F0-9]?[a-fA-F0-9]?[a-fA-F0-9]?|0)\:){6}(?:[a-fA-F0-9][a-fA-F0-9]?[a-fA-F0-9]?[a-fA-F0-9]?|0)$/;
      if(ipV6RegX.test(IP)) return "IPv6"; 
    return "Neither";
};
